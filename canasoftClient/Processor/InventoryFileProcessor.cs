...
public class InventoryFileProcessor : IFileProcessor
{
    private readonly IItemSource<CreateInventoryItemRequest> _source;
    private readonly IItemApiClient<CreateInventoryItemRequest> _apiClient;
        ...
        IOptionsMonitor<FileMappingConfig> optionsMonitor)
    {
        ...
        _config = optionsMonitor.Get("Inventory");
    }
    ...
    public async Task ProcessAsync(string filePath, ILogger logger)
    {
        ...
        foreach (var item in items)
        {
            ...
            {
                logger.LogError(ex, "Error on loading Inventory item: {Item}", item);
            }
        }
